...
local ICON_TEMPLATE = script:GetCustomProperty("MinimapIcon")
local NPC_ICON = script:GetCustomProperty("MinimapNPC")

local worldShapes = ROOT:FindDescendantsByType("StaticMesh")
local worldTexts = ROOT:FindDescendantsByType("WorldText")
local worldDecals = ROOT:FindDescendantsByType("Decal")
...
-- Icons
for _, decal in ipairs(worldDecals) do
	decal.isEnabled = false
	
	local pos = decal:GetWorldPosition()
	local rot = decal:GetWorldRotation()
	local size = decal:GetWorldScale() * 100	
	local icon = World.SpawnAsset(ICON_TEMPLATE, {parent = MAP_PANEL})	

	icon.x = (pos.x - boundsLeft) * scaleX
	icon.y = (pos.y - boundsTop) * scaleY	
	icon.width = 30
	icon.height = 30
	icon:SetColor(decal:GetSmartProperty("Color"))
end
...
function GetIndicatorForNPC(npc)
	if npc.clientUserData.minimap then
		return npc.clientUserData.minimap
	end
	-- Spawn new indicator for this player
	local minimapNPC = World.SpawnAsset(NPC_ICON, {parent = MAP_PANEL})
	npc.clientUserData.minimap = minimapNPC
	return minimapNPC
end
